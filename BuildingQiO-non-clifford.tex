...
\end{aligned}
\end{equation}
where each $\beta_i\in\mathbb{C}$  and is computed by multiplying and adding numbers from the set $\{\frac{1+i}{2},\frac{1-i}{2}\}.$ It is discussed in (\Cref{coeff:size}) that the size of the coefficients $\beta_i$'s grows at most a polynomial in the number of $\tgate$ gates. It follows from  \Cref{eq:pgate8} that the correction unitary for any 1-qubit quantum circuit can be written as 
...
\end{aligned}
\end{equation}
After expanding the relation \Cref{nqubit:eq8} using the identity $(A+B)\otimes C=A\otimes C + B\otimes C$.\footnote{We have expand the expression, in case if $\cnot$ acts on the wires that has $\tgate$ gates.} %\Anne{i don't understand this.}
