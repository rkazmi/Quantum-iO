...
\subsection*{Outline}
In section \ref{sec:QiO-Cliffords and more} We first define the notion of equivalent circuit, then we provide a definition of quantum indistinguishability obfuscation. We also show  how to construct quantum indistinguishability obfuscation for Clifford circuits from two different methodologies: One is based on canonical form of Clifford circuits while other is based on the principle of gate teleportation and a quantum secure $i\mathcal{O}.$ In section \ref{QiO:Clifford+T:family} we show that how to construct quantum indistinguishability obfuscation for any reversible quantum circuit that has at most logarithmic $\tgate$-gates in the size of the circuit.
