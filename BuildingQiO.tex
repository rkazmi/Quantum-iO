...
\subsection{$Qi\mathcal{O}$ for $n$-qubit Circuits}
...
{\em Update Function for arbitrary $n$-Qubit circuits}
...
in the relation \ref{nqubit:eq16}, while the R.H.S is equal to a {\bf 0} matrix. Therefore $m=t$ and since the terms our in  lexicographic order we must have  

 $\beta_{k_i}\xgate^{b_{k_{i1}}} \zgate^{a_{k_{i1}}} \otimes \cdots  \otimes \xgate^{b_{k_{in}}} \zgate^{a_{k_{in}}}$ t
