...
Hence, it follows from the above that by applying the remaining  sequence of gates $(g_{|C_q|},\ldots,g_3)$, the expression \ref{eq:pgate7} can be written as 
\begin{equation}
\begin{aligned}
\label{eq:pgate8}
...
%Let $C_q$ and $C_q^\prime$ be two 1-qubit equivalent circuits and $\beta_1 \igate+\beta_2 \xgate+\beta_3 \zgate+\beta_4 \xgate \zgate$ and $\beta^\prime_1 \igate+\beta^\prime_2 \xgate+\beta^\prime_3 \zgate+\beta^\prime_4 \xgate \zgate$ are the corresponding correction unitary matrices, then $\beta_i=\beta_i^\prime$  for all integers $1\leq i\leq 4.$
\end{lemma}
...
%\begin{equation}
%\label{eq:qubit1}
% \beta_1 \igate+\beta_2 \xgate+\beta_3 \zgate+\beta_4 \xgate \zgate C_q (\ket{\psi})=\beta^\prime_1 \igate+\beta^\prime_2 \xgate+\beta^\prime_3 \zgate+\beta^\prime_4 \xgate \zgate  C_q^\prime (\ket{\psi}).
% \end{equation}
% since $C_q (\ket{\psi})=C_q^\prime (\ket{\psi})$ we can substitute $C_q^\prime (\ket{\psi})$ in equation \ref{eq:qubit1} with $C_q (\ket{\psi}).$
%
% 
% \begin{equation}
% \label{eq:qubit2}
% \beta_1 \igate+\beta_2 \xgate+\beta_3 \zgate+\beta_4 \xgate \zgate (C_q (\ket{\psi}))=\beta^\prime_1 \igate+\beta^\prime_2 \xgate+\beta^\prime_3 \zgate+\beta^\prime_4 (C_q (\ket{\psi})).
%  \end{equation}
%  
%  From  equation \ref{eq:qubit2} we have 
% \begin{equation}
% \begin{aligned}
% \label{eq:qubit2}
%\beta_1 \igate+\beta_2 \xgate+\beta_3 \zgate+\beta_4 \xgate \zgate =\beta^\prime_1 \igate+\beta^\prime_2 \xgate+\beta^\prime_3 \zgate+\beta^\prime_4\\
%  =(\beta_1-\beta_1^\prime) \igate+(\beta_2 -\beta_2^\prime)\xgate+(\beta_3-\beta_3^\prime) \zgate+(\beta_4-\beta^\prime_4) \xgate \zgate={\bf 0}.
%  \end{aligned}
% \end{equation}
%  
%If for any $i,$ $\beta_i \neq \beta_i^\prime$ then the expression on the L.H.S cannot be equal to the zero matrix, therefore we must have $\beta_i =\beta_i^\prime$ for all integers $1\leq i\leq 4.$
...
%\item[4)] $\tgate(\alpha_1\xgate^{b_1}\zgate^{a_1\oplus b_1 } + \alpha_2\xgate^{b_1}\zgate^{a_1}) \hgate \tgate \textsf{G}$\\
%               $=\left(\frac{i}{2}\xgate^{a_1}\zgate^{a_1\oplus b_1 } +  \frac{(-1)^{b_1}}{2}\xgate^{a_1\oplus b_1}\zgate^{b_1}+
...
%
% \hspace{2.3in}   $=\left(c_1^\prime\igate+ c_2^\prime\xgate+c_3^\prime\zgate+c_4^\prime\xgate\zgate\right)\tgate \xgate\zgate\hgate\tgate\hgate \tgate \textsf{G},$ \\
%  where $c_1^\prime=c_1,$  $c_2^\prime=(c_2\alpha_2+c_4\alpha_1),$ $c_3^\prime=c_3,$ and $c_4^\prime=(c_2\alpha_1+c_4\alpha_2).$
%  %$\tgate(\left(c_1\igate+ c_2\xgate+c_3\zgate+c_4\xgate\zgate\right)\hgate\tgate\hgate \tgate \textsf{G},\right)$
%\item[-] Note applying any number gates after step 6 cannot increase the number of terms in the sum, but can only change the coefficients $c_i^\prime.$ In general, for any $1$-qubit  circuit the correction unitary is of form
%$$\left(\beta_1 \igate+\beta_2 \xgate+\beta_3 \zgate+\beta_4 \xgate \zgate\right), \; \mbox{ where } \beta_i\in \mathbb{C}.$$
%\end{enumerate}
%Therefore, for any arbitrary 1-qubit unitary circuit $C_q,$ the update function is $$F_{C_q}: \mathbb{F}_2^2 \longrightarrow \mathbb{C}^4,\hspace{0.5cm} (a,b)\longrightarrow (\beta_1,\beta_2,\beta_3, \beta_4).$$
...
%How big  are the $\beta_i's $? Note each $\beta_i's$ are generated by multiplying and adding elements from the set $\{\left(\frac{1+i}{2}\right), \left(\frac{1-i}{2}\right)\}$ and only $\tgate$ can have impact on the size of the coefficients in the summation. Therefore, the size of complex numbers grows polynomially in the number of $\tgate$-gates.
%
%
